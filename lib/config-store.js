...
module.exports.create = function (DirStore, RegStore, KeyStore) {
  ...
  function signRegistration(appname, privkey, providerUri, options) {
    ...
    , sub: providerUri          // rfc7519 for prn - the subject (principle) - in this case is the 3rd party login
    , typ: 'credentials'        // how to know what

    , data: options /*params*/  // non-spec, application specific
  ...
  function registerClientHelper(dir, hostnameUri, providerUri, options) {
    ...
    return KeyStore.getAsync(hostnameUri).then(function (privkeys) {
      var privkey = privkeys[0]; // TODO how to choose a particular key
      var token = signRegistration(hostnameUri, privkey, providerUri, options);
