  ...
  var TokenSigner = {
    ...
    signAsync: function (meta, params) {
      ...
      */
      ...
      var appname = (meta.referer || meta.origin || '').replace(re, '$2$4').replace(/\/$/, '');
      var appname2 = (meta.hostname || '').replace(re, '$2$4').replace(/\/$/, '');
      ...
      return DirStore.getAsync(meta.providerUri).then(getProfileUrl).then(parseAccountIds).then(function (profiles) {
        ...
        return KeyStore.getAsync(appname || appname2).then(function (privkeys) {
