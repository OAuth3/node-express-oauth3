...
module.exports.create = function (OAuth3Strategy, oauth3, options) {
  ...
  , authorizationRedirect: function (req, res) {
      OAuth3Strategy.authorizationRedirect(oauth3, req).then(function (redirectUrl) {
        //console.log('DEBUG authorizationRedirect res.redirect', redirectUrl);
  ...
  , authorizationCodeCallback: function (req, res) {
      OAuth3Strategy.authorizationCodeCallback(oauth3, req).then(function (result) {
        ...
        var baseUrl = (options.frontend || err.referer).replace(/\/$/, '');
        var result = {
          error: req.query.error || err.code || err.message
        , error_description: req.query.error_description || err.message
        , error_uri: req.query.error_uri || ('https://oauth3.org/docs/errors#' + (err.code || 'E_UNKNOWN_EXCEPTION_2'))
        , browser_state: err.browserState
        , state: err.browserState // [deprecated] 'state' may refer to the server state, so browser_state should be used
        };

        if (options.frontend || err.referer) {
          res.redirect(baseUrl + '/oauth3.html#' + querystring.stringify(result));
        } else {
          res.send(result);
        }
      }).catch(function (err) {
