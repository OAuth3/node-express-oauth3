...
function exchangeCodeForToken(self, req, oauth2, fullCallbackUrl/*, options*/) {
  ...
  return self._kv.getAsync(serverState).then(function (metaState) {
    ...
    // non-blocking async
    self._kv.destroyAsync(serverState);
    // TODO run in cluster.isMaster every 5 minutes
    self._kv.allAsync().then(function (all) {
      var now = Date.now();
      var item;
      ...
      function next() {
        ...
        if (now - item.createdAt > (15 * 60 * 1000)) {
          return self._kv.destroyAsync(item.serverState).then(next);
        }
